...
$("#todayApts").on("focusout", "tr .grommerNotes", function () {
  ...
  firebase
    .database()
    .ref("serviceRecord/" + noteServiceKey)
    .update({
...
$("#todayApts").on("focusout", "tr .priceChange", function () {
  ...
  firebase
    .database()
    .ref("serviceRecord/" + priceServiceKey)
    .update({
      totalPrice: newPrice,
    });
  $("#todayApts").html("");
...
$("#searchCustomers").click(function () {
  ...
  var searchValue = $("#searchValue").val().toUpperCase();
  $("#customerInfo").html(
      ...
      if (!snap.val()) {
        ...
        snap.forEach(function (childSnapshot) {
          ...
          function findCurrentCust() {
            var prcnt = (100 / rowNumber) * customersDone;
            console.log(prcnt);
            firebase
              .database()
              .ref("customers/")
                  ...
                  snapCustomer.forEach(function (childSnapshot) {
                    ...
                    $("#todayApts").append(
                      '<tr><th class="moreInfoTd" scope="row">' +
                        rowNumber +
                        '</th><td class="moreInfoTd" value="' +
                        serviceKey +
                        '">' +
                        custFN +
                        " " +
                        custLN +
                        '</td><td class="moreInfoTd" value="' +
                        serviceKey +
                        '">' +
                        petName +
                        '</td><td class="moreInfoTd" value="' +
                        ...
                        '" id="price' +
                        serviceKey +
                        '" class="priceChange" contenteditable="true">' +
                        totalPrice +
                        '</span></td><td id="td' +
                        serviceKey +
                        '"><select name=' +
                        serviceKey +
                        ' class="btn btn-dark" typy="button"><option disabled selected>Choose Groomer</option><option value="Melissa">Melissa</option><option value="Helen">Helen</option</select></td><td id="check' +
                        serviceKey +
                        '"><button id="btn' +
                        serviceKey +
                        '" value="' +
                        serviceKey +
                        '" type="button" class="btn btn-dark doneBtn">DONE</button></td></tr><tr id="more' +
                        serviceKey +
                        '"><td class="text-left" colspan="4"><b>Add-ons: </b>' +
                        checked +
                        '</td><td colspan="3" class="text-left"><b>Groomer notes: </b><span class="grommerNotes" value="' +
                        serviceKey +
                        '" contenteditable="true">' +
                        groomerNotes +
                        '</span></td><td><a id="historyButton" class="btn btn-dark" data-toggle="collapse" href="#his' +
                        customerKey +
                        '" role="button" aria-expanded="false">HISTORY</a></td></tr>'
                    );
                    if (groomerName == "Melissa" || groomerName == "Helen") {
                      $("#td" + serviceKey).html(groomerName);
                      $("#check" + serviceKey).html(
